<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="缴车日期" prop="jiaocheriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.jiaocheriqi" 
                type="date"
                :readonly="ro.jiaocheriqi"
                placeholder="缴车日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.jiaocheriqi" label="缴车日期" prop="jiaocheriqi">
              <el-input v-model="ruleForm.jiaocheriqi" 
                placeholder="缴车日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="报损明细" prop="baosunmingxi">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="报损明细"
                  v-model="ruleForm.baosunmingxi" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.baosunmingxi" label="报损明细" prop="baosunmingxi">
                    <span>{{ruleForm.baosunmingxi}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.chepaihao = true;
          ...
          if(o=='jiaocheriqi'){
            this.ruleForm.jiaocheriqi = obj[o];
	    this.ro.jiaocheriqi = true;
            continue;
          }
          if(o=='baosunmingxi'){
            this.ruleForm.baosunmingxi = obj[o];
	    this.ro.baosunmingxi = true;
            continue;
          }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiaochexinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiaochexinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
