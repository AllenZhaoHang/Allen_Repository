<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="资源类型" prop="ziyuanleixing">
          <el-input v-model="ruleForm.ziyuanleixing" 
              placeholder="资源类型" clearable  :readonly="ro.ziyuanleixing"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="资源类型" prop="ziyuanleixing">
              <el-input v-model="ruleForm.ziyuanleixing" 
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.ziyuanwenjian" label="资源文件" prop="ziyuanwenjian">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.ziyuanwenjian)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="备份时间" prop="beifenshijian">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.beifenshijian" 
                type="date"
                :readonly="ro.beifenshijian"
                placeholder="备份时间">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.beifenshijian" label="备份时间" prop="beifenshijian">
              <el-input v-model="ruleForm.beifenshijian" 
                placeholder="备份时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="资源简介" prop="ziyuanjianjie">
                <el-input
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.ziyuanjianjie" label="资源简介" prop="ziyuanjianjie">
                    <span>{{ruleForm.ziyuanjianjie}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.beifenshijian = this.getCurDate()
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.ziyuanjianjie = true;
          ...
          if(o=='beifenshijian'){
            this.ruleForm.beifenshijian = obj[o];
	    this.ro.beifenshijian = true;
            continue;
          }
        }
      }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `ziyuanbeifen/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.ziyuanbeifenCrossAddOrUpdateFlag = false;
						 this.parent.search();
