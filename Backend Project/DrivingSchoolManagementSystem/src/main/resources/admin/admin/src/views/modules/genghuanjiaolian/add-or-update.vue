<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="更换教练" prop="genghuanjiaolian">
          <el-select :disabled="ro.genghuanjiaolian" v-model="ruleForm.genghuanjiaolian" placeholder="请选择更换教练">
            <el-option
                v-for="(item,index) in genghuanjiaolianOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="更换教练" prop="genghuanjiaolian">
	      <el-input v-model="ruleForm.genghuanjiaolian"
                placeholder="更换教练" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="更换原因" prop="genghuanyuanyin">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="更换原因"
                  v-model="ruleForm.genghuanyuanyin" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.genghuanyuanyin" label="更换原因" prop="genghuanyuanyin">
                    <span>{{ruleForm.genghuanyuanyin}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	genghuanjiaolian : false,
	shenqingshijian : false,
	genghuanyuanyin : false,
	zhanghao : false,
	xingming : false,
	sfsh : false,
	shhf : false,
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.jiaolianxingming = true;
          ...
          if(o=='genghuanjiaolian'){
            this.ruleForm.genghuanjiaolian = obj[o];
	    this.ro.genghuanjiaolian = true;
            continue;
          }
	    ...
	    this.ro.shenqingshijian = true;
          ...
          if(o=='genghuanyuanyin'){
            this.ruleForm.genghuanyuanyin = obj[o];
	    this.ro.genghuanyuanyin = true;
            continue;
          }
          if(o=='zhanghao'){
      ...
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/jiaolian/jiaolianxingming`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.genghuanjiaolianOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `genghuanjiaolian/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.genghuanjiaolianCrossAddOrUpdateFlag = false;
						 this.parent.search();
